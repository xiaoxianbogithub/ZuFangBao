...
public class ResidenceIdPrefixTask {
    ...
    public void setRedisCacheId() {
        ...
        redisCache.setCacheObject(CacheConstants.RESIDENCE_ID_KEY + CharSequenceUtil.format(CacheConstants.RESIDENCE_ID_KEY_TEMPLATE, Constants.RESIDENCE_PREFIX_ZY, dateStr), 0, 24, TimeUnit.HOURS);
        // 设置品牌经纪人房源Id redis key, 值为 0, 有效期为 24h
        redisCache.setCacheObject(CacheConstants.RESIDENCE_ID_KEY + CharSequenceUtil.format(CacheConstants.RESIDENCE_ID_KEY_TEMPLATE, Constants.RESIDENCE_PREFIX_PP, dateStr), 0, 24, TimeUnit.HOURS);
        // 设置自由经纪人房源Id redis key, 值为 0, 有效期为 24h
        redisCache.setCacheObject(CacheConstants.RESIDENCE_ID_KEY + CharSequenceUtil.format(CacheConstants.RESIDENCE_ID_KEY_TEMPLATE, Constants.RESIDENCE_PREFIX_FY, dateStr), 0, 24, TimeUnit.HOURS);
        // log 记录
